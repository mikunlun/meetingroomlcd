...
/**
 * This is from gist: The stock dropwizard ServerCommand blocks the thread that
 * starts the server, and provides no way to stop the service.
 * <p/>
 * EmbeddedServerCommand runs jetty on a separate thread and provides a stop
 * method.
 * <p/>
 * This class is based on: TestableServerCommand by Kim A. Betti
 * <kim@developer-b.com> https://gist.github.com/2789987 (This class from:
 * https:
 * //github.com/alexspurling/dropwizard-ci/blob/master/src/main/java/org/pio
 * /dropwizard/EmbeddedServerCommand.java)
 */
public class EmbeddedServerCommand<T extends Configuration> extends EnvironmentCommand<T> {
	...
	/**
	 * We won't be able to run more than a single test in the same JVM instance
	 * unless we do some tidying and un-register a logging m-bean added by
	 * Dropwizard.
	 */
