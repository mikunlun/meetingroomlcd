...
class SyncFolderItemsRequest extends MultiResponseServiceRequest<SyncFolderItemsResponse> {
  ...
  protected void validate() throws Exception {
    ...
    if (this.getService().getRequestedServerVersion().compareTo(ExchangeVersion.Exchange2010) < 0 && this.syncScope != SyncFolderItemsScope.NormalItems) {
      throw new ServiceVersionException(String.format(Strings.EnumValueIncompatibleWithRequestVersion, this.getSyncScope().toString(), this.getSyncScope().name(), ExchangeVersion.Exchange2010));
    }

    // SyncFolderItems can only handle summary properties
    this.getPropertySet().validateForRequest(this, true /* summaryPropertiesOnly */);
  }
  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer) throws Exception {
    ...
    writer.writeElementValue(XmlNamespace.Messages, XmlElementNames.SyncState, this.getSyncState());

    this.getIgnoredItemIds().writeToXml(writer, XmlNamespace.Messages, XmlElementNames.Ignore);

    writer.writeElementValue(XmlNamespace.Messages, XmlElementNames.MaxChangesReturned, this.getMaxChangesReturned());
