...
public final class PropertySet implements ISelfValidate, Iterable<PropertyDefinitionBase> {
  ...
  public void addRange(Iterable<PropertyDefinitionBase> properties) throws Exception {
    ...
    for (Iterator<PropertyDefinitionBase> it = properties.iterator(); it.hasNext(); ) {
  ...
  protected void validateForRequest(ServiceRequestBase request, boolean summaryPropertiesOnly) throws ServiceVersionException, ServiceValidationException {
    for (PropertyDefinitionBase propDefBase : this.additionalProperties) {
      if (propDefBase instanceof PropertyDefinition) {
        ...
        if (summaryPropertiesOnly && !propertyDefinition.hasFlag(PropertyDefinitionFlags.CanFind, request.
  ...
  protected void writeToXml(EwsServiceXmlWriter writer, ServiceObjectType serviceObjectType) throws XMLStreamException, ServiceXmlSerializationException {
    writer.writeStartElement(XmlNamespace.Messages, serviceObjectType == ServiceObjectType.Item ? XmlElementNames.ItemShape : XmlElementNames.FolderShape);
    ...
    if (serviceObjectType == ServiceObjectType.Item) {
      ...
      if (this.getFilterHtmlContent() != null) {
        writer.writeElementValue(XmlNamespace.Types, XmlElementNames.FilterHtmlContent, this.getFilterHtmlContent());
      }
      if ((this.getConvertHtmlCodePageToUTF8() != null) && writer.getService().getRequestedServerVersion().
              compareTo(ExchangeVersion.Exchange2010_SP1) >= 0) {
        writer.writeElementValue(XmlNamespace.Types, XmlElementNames.ConvertHtmlCodePageToUTF8, this.getConvertHtmlCodePageToUTF8());
      }
    }

    if (this.additionalProperties.size() > 0) {
      writeAdditionalPropertiesToXml(writer, this.additionalProperties.iterator());
    }
