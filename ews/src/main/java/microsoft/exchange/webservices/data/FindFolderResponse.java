...
final class FindFolderResponse extends ServiceResponse {
  ...
  protected void readElementsFromXml(EwsServiceXmlReader reader) throws Exception {
    ...
    this.results.setTotalCount(reader.readAttributeValue(Integer.class, XmlAttributeNames.TotalItemsInView));
    this.results.setMoreAvailable(!reader.readAttributeValue(Boolean.class, XmlAttributeNames.IncludesLastItemInRange));

    // Ignore IndexedPagingOffset attribute if MoreAvailable is false.
    this.results.setNextPageOffset(results.isMoreAvailable() ? reader.readNullableAttributeValue(Integer.class, XmlAttributeNames.IndexedPagingOffset) : null);
    ...
    if (!reader.isEmptyElement()) {
      do {
        ...
        if (reader.getNodeType().nodeType == XMLNodeType.START_ELEMENT) {
          Folder folder = EwsUtilities.createEwsObjectFromXmlElementName(Folder.class, reader.getService(), reader.getLocalName());
