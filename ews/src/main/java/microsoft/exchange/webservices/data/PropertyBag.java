...
class PropertyBag implements IComplexPropertyChanged, IComplexPropertyChangedDelegate {
   ...
   * @throws ArgumentException
   */
  protected <T> boolean tryGetPropertyType(Class<T> cls, PropertyDefinition propertyDefinition, OutParam<T> propertyValue) throws ArgumentException {
   ...
   */
  private Object getPropertyValueOrException(PropertyDefinition propertyDefinition, OutParam<ServiceLocalException> serviceExceptionOutParam) {
    ...
    if (propertyDefinition.getVersion().ordinal() > this.getOwner().getService().getRequestedServerVersion().ordinal()) {
      serviceExceptionOutParam.setParam(new ServiceVersionException(String.format(Strings.PropertyIncompatibleWithRequestVersion, propertyDefinition.getName(), propertyDefinition.getVersion())));
      return null;
    }
    ...
    if (this.tryGetValue(propertyDefinition, propertyValueOutParam)) {
      ...
      if (propertyDefinition.hasFlag(PropertyDefinitionFlags.AutoInstantiateOnRead)) {
        EwsUtilities.EwsAssert(propertyDefinition instanceof ComplexPropertyDefinitionBase, "PropertyBag.get_this[]", "propertyDefinition is " +
        ...
        Object propertyValue = complexPropertyDefinition.createPropertyInstance(this.getOwner());
        ...
        if (propertyDefinition != this.getOwner().getIdPropertyDefinition()) {
          if (!this.isPropertyLoaded(propertyDefinition)) {
            serviceExceptionOutParam.setParam(new ServiceObjectPropertyException(Strings.
                    MustLoadOrAssignPropertyBeforeAccess, propertyDefinition));
            return null;
          }
          ...
          if (!propertyDefinition.isNullable()) {
            String errorMessage = this.isRequestedProperty(propertyDefinition) ? Strings.ValuePropertyNotLoaded : Strings.ValuePropertyNotAssigned;
            serviceExceptionOutParam.setParam(new ServiceObjectPropertyException(errorMessage, propertyDefinition));
          }
        }
      }
      return propertyValueOutParam.getParam();
   ...
   */
  protected boolean tryGetValue(PropertyDefinition propertyDefinition, OutParam<Object> propertyValueOutParam) {
    if (this.properties.containsKey(propertyDefinition)) {
      propertyValueOutParam.setParam(this.properties.get(propertyDefinition));
      return true;
    } else {
      propertyValueOutParam.setParam(null);
      return false;
  ...
  protected void propertyChanged(ComplexProperty complexProperty) {
    Iterator<Entry<PropertyDefinition, Object>> it = this.properties.entrySet().iterator();
    while (it.hasNext()) {
      ...
      if (keyValuePair.getValue().equals(complexProperty)) {
        if (!this.deletedProperties.containsKey(keyValuePair.getKey())) {
          addToChangeList(keyValuePair.getKey(), this.modifiedProperties);
          this.changed();
        }
      }
    }
   ...
   */
  protected void loadFromXml(EwsServiceXmlReader reader, boolean clear, PropertySet requestedPropertySet, boolean onlySummaryPropertiesRequested) throws Exception {
    if (clear) {
      this.clear();
    }
  ...
  protected void writeToXml(EwsServiceXmlWriter writer) throws Exception {
    ...
    Iterator<PropertyDefinition> it = this.getOwner().getSchema().iterator();
    while (it.hasNext()) {
  ...
  protected void writeToXmlForUpdate(EwsServiceXmlWriter writer) throws Exception {
    ...
    Iterator<Entry<PropertyDefinition, Object>> it = this.deletedProperties.entrySet().iterator();
    while (it.hasNext()) {
   ...
   */
  protected Object getObjectFromPropertyDefinition(PropertyDefinition propertyDefinition) throws ServiceLocalException {
    ...
    Object propertyValue = this.getPropertyValueOrException(propertyDefinition, serviceExceptionOut);
   ...
   * @throws Exception the exception
   */
  protected void setObjectFromPropertyDefinition(PropertyDefinition propertyDefinition, Object object) throws Exception {
    if (propertyDefinition.getVersion().ordinal() > this.getOwner().getService().getRequestedServerVersion().ordinal()) {
