...
class AutodiscoverDnsClient {
  ...
  protected String findAutodiscoverHostFromSrv(String domain) throws XMLStreamException, IOException {
    ...
    if ((dnsSrvRecord == null) || dnsSrvRecord.getNameTarget() == null ||
      ...
      this.service.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("DNS query for SRV record for domain %s found %s", domain, dnsSrvRecord.getNameTarget()));
  ...
  private DnsSrvRecord findBestMatchingSrvRecord(String domain) throws XMLStreamException, IOException {
    ...
    try {
      ...
      String dnsExcMessage = String.format("DnsQuery returned error " + "error '%s' error code 0x{1:X8}.", ex.getMessage(), ex.getError());
    ...
    String traceMessage = String.format("Returning SRV record %d " +
            "of %d records. " +
            "Target: %s, Priority: %d, Weight: %d", recordIndex, dnsSrvRecordList.size(), bestDnsSrvRecord.getNameTarget(), bestDnsSrvRecord.getPriority(), bestDnsSrvRecord.getWeight());
