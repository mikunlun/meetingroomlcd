...
final class FindConversationResponse extends ServiceResponse {
  ...
  protected void readElementsFromXml(EwsServiceXmlReader reader) throws Exception {
    EwsUtilities.EwsAssert(conversations != null, "FindConversationResponse.ReadElementsFromXml", "conversations is null.");
      ...
      do {
        ...
        if (reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) {
          Conversation item = EwsUtilities.
                  createEwsObjectFromXmlElementName(Conversation.class, reader.getService(), reader.getLocalName());

          if (item == null) {
            reader.skipCurrentElement();
          } else {
            item.loadFromXml(reader, true, /* clearPropertyBag */
                    null, false  /* summaryPropertiesOnly */);
