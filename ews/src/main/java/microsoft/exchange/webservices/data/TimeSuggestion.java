...
public final class TimeSuggestion extends ComplexProperty {
  ...
  protected boolean tryReadElementFromXml(EwsServiceXmlReader reader) throws Exception {
    if (reader.getLocalName().equals(XmlElementNames.MeetingTime)) {
      ...
      return true;
    } else if (reader.getLocalName().equals(XmlElementNames.SuggestionQuality)) {
      this.quality = reader.readElementValue(SuggestionQuality.class);
      return true;
    } else if (reader.getLocalName().equals(XmlElementNames.AttendeeConflictDataArray)) {
      if (!reader.isEmptyElement()) {
        do {
          ...
          if (reader.isStartElement()) {
            ...
            if (reader.getLocalName().equals(XmlElementNames.UnknownAttendeeConflictData)) {
              conflict = new Conflict(ConflictType.UnknownAttendeeConflict);
            } else if (reader.getLocalName().equals(XmlElementNames.
            ...
            } else if (reader.getLocalName().equals(XmlElementNames.GroupAttendeeConflictData)) {
              conflict = new Conflict(ConflictType.GroupConflict);
            } else {
              EwsUtilities.EwsAssert(false, "TimeSuggestion." + "TryReadElementFromXml", String.format("The %s element name " +
                      "does not map " +
