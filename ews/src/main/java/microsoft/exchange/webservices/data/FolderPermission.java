...
public final class FolderPermission extends ComplexProperty implements IComplexPropertyChangedDelegate {
  ...
  private static LazyMember<Map<FolderPermissionLevel, FolderPermission>> defaultPermissions = new LazyMember<Map<FolderPermissionLevel, FolderPermission>>(new ILazyMember<Map<FolderPermissionLevel, FolderPermission>>() {
    ...
    public Map<FolderPermissionLevel, FolderPermission> createInstance() {
      Map<FolderPermissionLevel, FolderPermission> result = new HashMap<FolderPermissionLevel, FolderPermission>();
      ...
      result.put(FolderPermissionLevel.
              FreeBusyTimeAndSubjectAndLocation, permission);
      return result;
    }
  });
  //End Region
  ...
  private static LazyMember<List<FolderPermission>> levelVariants = new LazyMember<List<FolderPermission>>(new ILazyMember<List<FolderPermission>>() {
    ...
    public List<FolderPermission> createInstance() {
      List<FolderPermission> results = new ArrayList<FolderPermission>();

      FolderPermission permissionNone = FolderPermission.
              defaultPermissions.getMember().get(FolderPermissionLevel.None);
      FolderPermission permissionOwner = FolderPermission.
              defaultPermissions.getMember().get(FolderPermissionLevel.Owner);
  ...
  private void AdjustPermissionLevel() {
    for (Entry<FolderPermissionLevel, FolderPermission> keyValuePair : defaultPermissions.getMember().entrySet()) {
  ...
  void validate(boolean isCalendarFolder, int permissionIndex) throws ServiceValidationException, ServiceLocalException {
    ...
    // values
    if (!isCalendarFolder) {
      if ((this.readItems == FolderPermissionReadAccess.TimeAndSubjectAndLocation) || (this.readItems == FolderPermissionReadAccess.
              TimeOnly)) {
        throw new ServiceLocalException(String.format(Strings.ReadAccessInvalidForNonCalendarFolder, this.readItems));
      }

      if ((this.permissionLevel == FolderPermissionLevel.FreeBusyTimeAndSubjectAndLocation) || (this.permissionLevel == FolderPermissionLevel.
              FreeBusyTimeOnly)) {
  ...
  public void setPermissionLevel(FolderPermissionLevel value) throws ServiceLocalException {
    if (this.permissionLevel != value) {
      ...
      this.AssignIndividualPermissions(defaultPermissions.getMember().get(value));
  ...
  public FolderPermissionLevel getDisplayPermissionLevel() {
    ...
    if (this.permissionLevel == FolderPermissionLevel.Custom) {
      for (FolderPermission variant : FolderPermission.levelVariants.getMember()) {
  ...
  protected boolean tryReadElementFromXml(EwsServiceXmlReader reader) throws Exception {
    if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.UserId)) {
      ...
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.CanCreateItems)) {
      this.canCreateItems = reader.readValue(Boolean.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.CanCreateSubFolders)) {
      this.canCreateSubFolders = reader.readValue(Boolean.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsFolderOwner)) {
      this.isFolderOwner = reader.readValue(Boolean.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsFolderVisible)) {
      this.isFolderVisible = reader.readValue(Boolean.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsFolderContact)) {
      this.isFolderContact = reader.readValue(Boolean.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.EditItems)) {
      this.editItems = reader.readValue(PermissionScope.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.DeleteItems)) {
      this.deleteItems = reader.readValue(PermissionScope.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.ReadItems)) {
      this.readItems = reader.readValue(FolderPermissionReadAccess.class);
      return true;
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.PermissionLevel) || reader.getLocalName().equalsIgnoreCase(XmlElementNames.CalendarPermissionLevel)) {
  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer, boolean isCalendarFolder) throws Exception {
    ...
    writer.writeElementValue(XmlNamespace.Types, isCalendarFolder ? XmlElementNames.
            CalendarPermissionLevel : XmlElementNames.PermissionLevel, this.permissionLevel);
  }
