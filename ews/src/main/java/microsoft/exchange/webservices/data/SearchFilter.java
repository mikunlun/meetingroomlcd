...
public abstract class SearchFilter extends ComplexProperty {
   ...
   * @throws Exception the exception
   */
  protected static SearchFilter loadFromXml(EwsServiceXmlReader reader) throws Exception {
    ...
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.Contains)) {
      searchFilter = new ContainsSubstring();
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.Excludes)) {
    ...
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsEqualTo)) {
      searchFilter = new IsEqualTo();
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsNotEqualTo)) {
      searchFilter = new IsNotEqualTo();
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsGreaterThan)) {
      searchFilter = new IsGreaterThan();
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsGreaterThanOrEqualTo)) {
      searchFilter = new IsGreaterThanOrEqualTo();
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsLessThan)) {
      searchFilter = new IsLessThan();
    } else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.IsLessThanOrEqualTo)) {
  ...
  public static final class ContainsSubstring extends PropertyBasedFilter {
    ...
    protected void readAttributesFromXml(EwsServiceXmlReader reader) throws Exception {
      ...
      this.containmentMode = reader.readAttributeValue(ContainmentMode.class, XmlAttributeNames.ContainmentMode);
      try {
        this.comparisonMode = reader.readAttributeValue(ComparisonMode.class, XmlAttributeNames.ContainmentComparison);
    ...
    protected void internalValidate() throws ServiceValidationException {
      ...
      if (this.otherPropertyDefinition == null && this.value == null) {
        ...
        if (!((value instanceof Object) || (value instanceof ISearchStringProvider))) {
          throw new ServiceValidationException(String.format(Strings.SearchFilterComparisonValueTypeIsNotSupported, value.getClass().getName()));
        }
      }
    }
  ...
  public static class SearchFilterCollection extends SearchFilter implements Iterable<SearchFilter>, IComplexPropertyChangedDelegate {
     ...
     * The search filters.
     */
    private ArrayList<SearchFilter> searchFilters = new ArrayList<SearchFilter>();
    ...
    public SearchFilterCollection(LogicalOperator logicalOperator, SearchFilter... searchFilters) {
      ...
      for (SearchFilter search : searchFilters) {
        Iterable<SearchFilter> searchFil = java.util.Arrays.asList(search);
    ...
    public void removeAt(int index) {
      if (index < 0 || index >= this.getCount()) {
        throw new IllegalArgumentException("index", new Throwable(Strings.IndexIsOutOfRange));
      }
    ...
    public SearchFilter getSearchFilter(int index) {
      if (index < 0 || index >= this.getCount()) {
        throw new IllegalArgumentException(Strings.IndexIsOutOfRange + ":" + index);
      }
