...
class EwsUtilities {
   ...
   */
  private static LazyMember<Map<Class<?>, Map<String, ExchangeVersion>>> enumVersionDictionaries = new LazyMember<Map<Class<?>, Map<String, ExchangeVersion>>>(new ILazyMember<Map<Class<?>, Map<String, ExchangeVersion>>>() {
    @Override
    public Map<Class<?>, Map<String, ExchangeVersion>> createInstance() {
      ...
      enumDicts.put(MeetingRequestsDeliveryScope.class, buildEnumDict(MeetingRequestsDeliveryScope.
              class));
      return enumDicts;
    }
  });
   ...
   */
  private static LazyMember<Map<Class<?>, Map<String, String>>> schemaToEnumDictionaries = new LazyMember<Map<Class<?>, Map<String, String>>>(new ILazyMember<Map<Class<?>, Map<String, String>>>() {
    @Override
   ...
   */
  protected static LazyMember<Map<Class<?>, Map<String, String>>> enumToSchemaDictionaries = new LazyMember<Map<Class<?>, Map<String, String>>>(new ILazyMember<Map<Class<?>, Map<String, String>>>() {
    @Override
    public Map<Class<?>, Map<String, String>> createInstance() {
      Map<Class<?>, Map<String, String>> enumDicts = new HashMap<Class<?>, Map<String, String>>();
  ...
  protected static <TServiceObject extends ServiceObject> TServiceObject createEwsObjectFromXmlElementName(Class<?> itemClass, ExchangeService service, String xmlElementName) throws Exception {
    ...
    if (EwsUtilities.serviceObjectInfo.getMember().getXmlElementNameToServiceObjectClassMap().containsKey(xmlElementName)) {
      itemClass = EwsUtilities.serviceObjectInfo.getMember().getXmlElementNameToServiceObjectClassMap().get(xmlElementName);
      if (EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithServiceParam().containsKey(itemClass)) {
        creationDelegate = EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithServiceParam().get(itemClass);
  ...
  protected static Item createItemFromItemClass(ItemAttachment itemAttachment, Class itemClass, boolean isNew) throws Exception {
    ...
    if (EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithAttachmentParam().containsKey(itemClass)) {

      creationDelegate = EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithAttachmentParam().get(itemClass);
  ...
  protected static String serializeEnum(Object value) {
    ...
    if (enumToSchemaDictionaries.getMember().
      ...
      enumToStringDict = enumToSchemaDictionaries.getMember().get(value.getClass());
      Enum<?> e = (Enum<?>) value;
...
//		long retval=1010010;
  ...
  public static String getPrintableTypeName(Class type) {
    ...
    if (genericArgs.length > 0) {
      ...
      String genericPrefix = type.getName().substring(0, type.getName().indexOf('`'));
      ...
      for (int i = 0; i < genericArgs.length; i++) {
        ...
        genericArgsStr.append(getPrintableTypeName(genericArgs[i].getClass()));
      }
      nameBuilder.append("<");
      ...
      String arrayPrefix = type.getName().substring(0, type.getName().indexOf('['));
      StringBuilder nameBuilder = new StringBuilder(EwsUtilities.
  ...
  protected static void validateEnumVersionValue(Enum<?> enumValue, ExchangeVersion requestVersion) throws ServiceVersionException {
    ...
    if (enumVersionDict.containsKey(enumValue.toString())) {
      ...
      if (i < 0) {
        throw new ServiceVersionException(String.format("%S,%S,%S,%S", Strings.EnumValueIncompatibleWithRequestVersion, enumValue.toString(), enumValue.getClass().getName(), enumVersion));
      }
    }
  }
  ...
  protected static <T> int getEnumeratedObjectCount(Iterator<T> objects) {
    ...
    while (objects != null && objects.hasNext()) {
      @SuppressWarnings("unused") Object obj = objects.next();
      count++;
    }
    return count;
  }
   ...
   *                   Entry type.
   * @param collection The collection.
   * @param predicate  Predicate that defines the conditions
