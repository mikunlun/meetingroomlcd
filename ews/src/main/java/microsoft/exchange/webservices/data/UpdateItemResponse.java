...
public final class UpdateItemResponse extends ServiceResponse implements IGetObjectInstanceDelegate {
  ...
  protected void readElementsFromXml(EwsServiceXmlReader reader) throws ServiceXmlDeserializationException, XMLStreamException, InstantiationException, IllegalAccessException, Exception {
    ...
    reader.readServiceObjectsCollectionFromXml(XmlElementNames.Items, this, false, null, false);
    ...
    if (!reader.getService().getExchange2007CompatibilityMode()) {
      ...
      this.conflictCount = reader.readElementValue(Integer.class, XmlNamespace.Types, XmlElementNames.Count);
    ...
    if (this.returnedItem != null) {
      if (this.item.getId().getUniqueId().equals(this.returnedItem.getId().getUniqueId())) {
  ...
  private Item getObjectInstance(ExchangeService service, String xmlElementName) throws Exception {
    this.returnedItem = EwsUtilities.createEwsObjectFromXmlElementName(Item.class, service, xmlElementName);
