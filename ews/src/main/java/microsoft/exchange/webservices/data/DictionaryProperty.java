...
public abstract class DictionaryProperty<TKey, TEntry extends DictionaryEntryProperty<TKey>> extends ComplexProperty implements ICustomXmlUpdateSerializer, IComplexPropertyChangedDelegate {
  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer) throws Exception {
    for (Entry<TKey, TEntry> keyValuePair : this.entries.entrySet()) {
      keyValuePair.getValue().writeToXml(writer, this.getEntryXmlElementName(keyValuePair.getValue()));
    }
  }
  ...
  public boolean writeSetUpdateToXml(EwsServiceXmlWriter writer, ServiceObject ewsObject, PropertyDefinition propertyDefinition) throws Exception {
    ...
    for (TEntry entry : tempEntries) {
      ...
      if (!entry.writeSetUpdateToXml(writer, ewsObject, propertyDefinition.getXmlElement())) {
