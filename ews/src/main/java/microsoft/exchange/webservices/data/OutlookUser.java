...
final class OutlookUser {
  ...
  protected void loadFromXml(EwsXmlReader reader) throws Exception {
    ...
    do {
      ...
      if (reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) {
        ...
        } else if (reader.getLocalName().equals(XmlElementNames.LegacyDN)) {
          this.legacyDN = reader.readElementValue();
        } else if (reader.getLocalName().equals(XmlElementNames.DeploymentId)) {
  ...
  protected void convertToUserSettings(List<UserSettingName> requestedSettings, GetUserSettingsResponse response) {
    ...
    Map<UserSettingName, IFunc<OutlookUser, String>> converterQuery = new HashMap<UserSettingName, IFunc<OutlookUser, String>>();
