...
public final class AutodiscoverService extends ExchangeServiceBase implements IAutodiscoverRedirectionUrl, IFunctionDelegate {
   ...
   */
  private static final String AutodiscoverLegacyUrl = "%s://%s" + AutodiscoverLegacyPath;
    ...
    if (AutodiscoverRequest.isRedirectionResponse(request)) {
      ...
      if (!(location == null || location.isEmpty())) {
        try {
          ...
          if ((redirectUrl.getParam().getScheme().toLowerCase().equals("https")) && (redirectUrl.getParam().getPath().equalsIgnoreCase(AutodiscoverLegacyPath))) {
            this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("Redirection URL found: '%s'", redirectUrl.getParam().toString()));
  ...
  private <TSettings extends ConfigurationSettingsBase> TSettings internalGetLegacyUserSettings(Class<TSettings> cls, String emailAddress, List<String> redirectionEmailAddresses, OutParam<Integer> currentHop) throws Exception {
    ...
    do {
      ...
      try {
        ...
        switch (settings.getResponseType()) {
          ...
          case RedirectUrl:
            if (currentHop.getParam() < AutodiscoverMaxRedirections) {
              ...
              this.traceMessage(TraceFlags.AutodiscoverResponse, String.format("Autodiscover " +
                      "service " +
                      "returned " +
                      "redirection URL '%s'.", settings.getRedirectTarget()));
          ...
          case RedirectAddress:
            if (currentHop.getParam() < AutodiscoverMaxRedirections) {
              ...
              this.traceMessage(TraceFlags.AutodiscoverResponse, String.format("Autodiscover " +
                      ...
                      "returned " +
                      "redirection email " +
                      "address '%s'.", settings.getRedirectTarget()));
        ...
        this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("%s failed: XML parsing error: %s", url, ex.getMessage()));

        // The content at the URL wasn't a valid response, let's try the
        // next.
        currentUrlIndex++;
      } catch (IOException ex) {
        this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("%s failed: I/O error: %s", url, ex.getMessage()));
        ...
        if ((response != null) && this.tryGetRedirectionResponse(response, outParam1)) {
          ...
          this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("%s failed: %s (%s)", url, ex.getClass().getName(), ex.getMessage()));
      ...
      for (int currentHop = 0; currentHop < AutodiscoverService.AutodiscoverMaxRedirections; currentHop++) {
        try {
            ...
            case RedirectUrl:
              try {
                ...
                this.traceMessage(TraceFlags.
                        ...
                        "invalid " +
                        "redirection " +
                        "URL %s", settings.getParam().getRedirectTarget()));
                return false;
              }
              break;
            default:
              String failureMessage = String.format("Autodiscover call at %s failed with error %s, target %s", redirectionUrl, settings.getParam().getResponseType(), settings.getParam().getRedirectTarget());
          ...
          if ((response != null) && this.tryGetRedirectionResponse(response, outParam)) {
            ...
            this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("Host returned a " +
                    "redirection" +
  ...
  protected GetUserSettingsResponseCollection getUserSettings(final List<String> smtpAddresses, List<UserSettingName> settings) throws Exception {
    ...
    return (GetUserSettingsResponseCollection) this.getSettings(GetUserSettingsResponseCollection.class, UserSettingName.class, smtpAddresses, settings, null, this, new IFuncDelegate<String>() {
  ...
  private GetUserSettingsResponseCollection internalGetUserSettings(List<String> smtpAddresses, List<UserSettingName> settings, ExchangeVersion requestedVersion, URI autodiscoverUrl) throws ServiceLocalException, Exception {
    ...
    this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("Maximum number of redirection hops %d exceeded", AutodiscoverMaxRedirections));
  ...
  protected GetDomainSettingsResponseCollection getDomainSettings(final List<String> domains, List<DomainSettingName> settings, ExchangeVersion requestedVersion) throws Exception {
    ...
    return (GetDomainSettingsResponseCollection) this.getSettings(GetDomainSettingsResponseCollection.class, DomainSettingName.class, domains, settings, requestedVersion, this, new IFuncDelegate<String>() {
      public String func() {
  ...
  private boolean tryGetAutodiscoverEndpointUrl(String host, OutParam<URI> url) throws Exception {
    ...
    if (this.tryGetEnabledEndpointsForHost(host, outParam)) {
      ...
      // available.
      if ((!endpoints.contains(AutodiscoverEndpoints.Soap)) && (!endpoints.contains(AutodiscoverEndpoints.WsSecurity))
    ...
    for (int currentHop = 0; currentHop < AutodiscoverMaxRedirections; currentHop++) {
      ...
      if (request != null) {
        ...
        if (this.tryGetRedirectionResponse(request, outParam)) {
          ...
          this.traceMessage(TraceFlags.AutodiscoverConfiguration, String.format("Host returned enabled endpoint flags: %s", endpoints.getParam().toString()));
