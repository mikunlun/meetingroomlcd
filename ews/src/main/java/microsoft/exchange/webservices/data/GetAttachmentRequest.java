 ...
 */
final class GetAttachmentRequest extends MultiResponseServiceRequest<GetAttachmentResponse> {
  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer) throws XMLStreamException, ServiceXmlSerializationException {
    if ((this.getBodyType() != null) || this.getAdditionalProperties().size() > 0) {
      ...
      if (this.getBodyType() != null) {
        writer.writeElementValue(XmlNamespace.Types, XmlElementNames.BodyType, this.getBodyType());
      }
    ...
    for (Attachment attachment : this.getAttachments()) {
      ...
      writer.writeAttributeValue(XmlAttributeNames.Id, attachment.getId());
