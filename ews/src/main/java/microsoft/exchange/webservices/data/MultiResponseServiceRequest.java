 ...
 */
abstract class MultiResponseServiceRequest<TResponse extends ServiceResponse> extends SimpleServiceRequestBase {
  ...
  protected Object parseResponse(EwsServiceXmlReader reader) throws Exception {
    ...
    if (serviceResponses.getCount() < this.getExpectedResponseMessageCount()) {
      if ((serviceResponses.getCount() == 1) && (serviceResponses.getResponseAtIndex(0).getResult() == ServiceResult.Error)) {
        throw new ServiceResponseException(serviceResponses.getResponseAtIndex(0));
      } else {
  ...
  protected ServiceResponseCollection<TResponse> execute() throws Exception {
    ...
    if (this.errorHandlingMode == ServiceErrorHandling.ThrowOnError) {
      EwsUtilities.EwsAssert(serviceResponses.getCount() == 1, "MultiResponseServiceRequest.Execute", "ServiceErrorHandling.ThrowOnError " + "error handling " +
