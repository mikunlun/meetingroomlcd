...
final class GetUserOofSettingsRequest extends SimpleServiceRequestBase {
  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer) throws XMLStreamException, ServiceXmlSerializationException {
    ...
    writer.writeElementValue(XmlNamespace.Types, XmlElementNames.Address, this.getSmtpAddress());
  ...
  protected Object parseResponse(EwsServiceXmlReader reader) throws Exception {
    ...
    if (serviceResponse.getErrorCode() == ServiceError.NoError) {
      ...
      serviceResponse.getOofSettings().setAllowExternalOof(reader.readElementValue(OofExternalAudience.class, XmlNamespace.Messages, XmlElementNames.AllowExternalOof));
    }
