...
abstract class ServiceRequestBase {
   ...
   */
  protected abstract void writeElementsToXml(EwsServiceXmlWriter writer) throws XMLStreamException, ServiceXmlSerializationException, ServiceLocalException, InstantiationException, IllegalAccessException, ServiceValidationException, Exception;
   ...
   * @throws Exception                 the exception
   */
  protected abstract Object parseResponse(EwsServiceXmlReader reader) throws ServiceXmlDeserializationException, XMLStreamException, InstantiationException, IllegalAccessException, ServiceLocalException, ServiceResponseException, IndexOutOfBoundsException, Exception;
  ...
  protected void writeToXml(EwsServiceXmlWriter writer) throws Exception {
    ...
    writer.writeAttributeValue("xmlns", EwsUtilities.getNamespacePrefix(XmlNamespace.Soap), EwsUtilities.getNamespaceUri(XmlNamespace.Soap));
    ...
    if (this.service.getPreferredCulture() != null) {
      writer.writeElementValue(XmlNamespace.Types, XmlElementNames.MailboxCulture, this.service.getPreferredCulture().getDisplayName());
    }

    /** Emit the DateTimePrecision header */

    if (this.getService().getDateTimePrecision().ordinal() != DateTimePrecision.Default.ordinal()) {
      writer.writeElementValue(XmlNamespace.Types, XmlElementNames.DateTimePrecision, this.getService().getDateTimePrecision().toString());
    }
  ...
  protected HttpWebRequest emit(OutParam<HttpWebRequest> request) throws Exception {
    ...
    this.getService().traceHttpRequestHeaders(TraceFlags.EwsRequestHttpHeaders, request.getParam());
   ...
   */
  private ByteArrayOutputStream getWebRequestStream(Future request) throws EWSHttpException, InterruptedException, ExecutionException {
  ...
  protected void traceResponse(HttpWebRequest request, ByteArrayOutputStream memoryStream) throws XMLStreamException, IOException, EWSHttpException {
    ...
    if (!isNullOrEmpty(contentType) && (contentType.startsWith("text/") || contentType.startsWith("application/soap"))) {
   ...
   */
  protected HttpWebRequest endGetEwsHttpWebResponse(OutParam<HttpWebRequest> request, AsyncRequestResult asyncResult) throws Exception {
  ...
  protected SoapFaultDetails readSoapFault(EwsServiceXmlReader reader) {
    ...
    try {
      ...
      reader.read();
      if (!reader.isStartElement() || (!reader.getLocalName().equals(XmlElementNames.SOAPEnvelopeElementName))) {
