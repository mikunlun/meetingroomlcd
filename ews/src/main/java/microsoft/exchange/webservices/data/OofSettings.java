...
public final class OofSettings extends ComplexProperty implements ISelfValidate {
  ...
  protected boolean tryReadElementFromXml(EwsServiceXmlReader reader) throws Exception {
    if (reader.getLocalName().equals(XmlElementNames.OofState)) {
      ...
      return true;
    } else if (reader.getLocalName().equals(XmlElementNames.ExternalAudience)) {
  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer) throws Exception {
    ...
    writer.writeElementValue(XmlNamespace.Types, XmlElementNames.OofState, this.getState());

    writer.writeElementValue(XmlNamespace.Types, XmlElementNames.ExternalAudience, this.getExternalAudience());

    if (this.getDuration() != null && this.getState() == OofState.Scheduled) {
      this.getDuration().writeToXml(writer, XmlElementNames.Duration);
    }

    this.serializeOofReply(this.getInternalReply(), writer, XmlElementNames.InternalReply);
    this.serializeOofReply(this.getExternalReply(), writer, XmlElementNames.ExternalReply);
  }
