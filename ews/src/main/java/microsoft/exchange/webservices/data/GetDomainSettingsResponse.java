...
public final class GetDomainSettingsResponse extends AutodiscoverResponse {
  ...
  protected void loadFromXml(EwsXmlReader reader, String endElementName) throws Exception {
    do {
      ...
      if (reader.getNodeType().nodeType == XMLNodeType.START_ELEMENT) {
        ...
        } else if (reader.getLocalName().equals(XmlElementNames.DomainSettingErrors)) {
          this.loadDomainSettingErrorsFromXml(reader);
        } else if (reader.getLocalName().equals(XmlElementNames.DomainSettings)) {
          try {
  ...
  protected void loadDomainSettingsFromXml(EwsXmlReader reader) throws Exception {
    if (!reader.isEmptyElement()) {
      do {
        ...
        if ((reader.getNodeType().nodeType == XMLNodeType.START_ELEMENT) && (reader.getLocalName().equals(XmlElementNames.DomainSetting))) {
          String settingClass = reader.readAttributeValue(XmlNamespace.XmlSchemaInstance, XmlAttributeNames.Type);

          if (settingClass.equals(XmlElementNames.DomainStringSetting)) {
  ...
  private void readSettingFromXml(EwsXmlReader reader) throws Exception {
    ...
    do {
      ...
      if (reader.getNodeType().nodeType == XMLNodeType.START_ELEMENT) {
        if (reader.getLocalName().equals(XmlElementNames.DomainStringSetting)) {
  ...
  private void loadDomainSettingErrorsFromXml(EwsXmlReader reader) throws Exception {
    if (!reader.isEmptyElement()) {
      do {
        ...
        if ((reader.getNodeType().nodeType == XMLNodeType.START_ELEMENT) && (reader.getLocalName().equals(XmlElementNames.DomainSettingError))) {
