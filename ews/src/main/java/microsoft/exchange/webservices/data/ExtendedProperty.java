  ...
  protected void writeElementsToXml(EwsServiceXmlWriter writer) throws ServiceXmlSerializationException, XMLStreamException {
    ...
    if (MapiTypeConverter.isArrayType(this.getPropertyDefinition().getMapiType())) {
      ...
      for (int index = 0; index <= array.size(); index++) {
        writer.writeElementValue(XmlNamespace.Types, XmlElementNames.Value, MapiTypeConverter.convertToString(this.getPropertyDefinition().getMapiType(), array.get(index)));
      }
      writer.writeEndElement();
    } else {
      writer.writeElementValue(XmlNamespace.Types, XmlElementNames.Value, MapiTypeConverter.convertToString(this.getPropertyDefinition().getMapiType(), this.getValue()));
    }
  }
