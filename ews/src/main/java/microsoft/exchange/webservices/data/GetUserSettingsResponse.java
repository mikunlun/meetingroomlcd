  ...
  protected void loadFromXml(EwsXmlReader reader, String endElementName) throws Exception {
    do {
      ...
      if (reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) {
        if (reader.getLocalName().equals(XmlElementNames.RedirectTarget)) {

          this.setRedirectTarget(reader.readElementValue());
        } else if (reader.getLocalName().equals(XmlElementNames.UserSettingErrors)) {
          this.loadUserSettingErrorsFromXml(reader);
        } else if (reader.getLocalName().equals(XmlElementNames.UserSettings)) {
  ...
  protected void loadUserSettingsFromXml(EwsXmlReader reader) throws Exception {
    if (!reader.isEmptyElement()) {
      do {
        ...
        if ((reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) && (reader.getLocalName().equals(XmlElementNames.UserSetting))) {
  ...
  private void readSettingFromXml(EwsXmlReader reader) throws Exception {
    ...
    do {
      ...
      if (reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) {
        ...
        } else if (reader.getLocalName().equals(XmlElementNames.WebClientUrls)) {

          value = WebClientUrlCollection.loadFromXml(reader);
        } else if (reader.getLocalName().equals(XmlElementNames.ProtocolConnections)) {
          value = ProtocolConnectionCollection.LoadFromXml(reader);
        } else if (reader.getLocalName().equals(XmlElementNames.AlternateMailboxes)) {
  ...
  private void loadUserSettingErrorsFromXml(EwsXmlReader reader) throws Exception {
    if (!reader.isEmptyElement()) {
      do {
        ...
        if ((reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) && (reader.getLocalName().equals(XmlElementNames.UserSettingError))) {
