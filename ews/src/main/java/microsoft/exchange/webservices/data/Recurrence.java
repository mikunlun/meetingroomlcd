...
public abstract class Recurrence extends ComplexProperty {
  ...
  public final static class RelativeMonthlyPattern extends IntervalPattern {
    ...
    protected boolean tryReadElementFromXml(EwsServiceXmlReader reader) throws Exception {
      if (super.tryReadElementFromXml(reader)) {
        ...
        if (reader.getLocalName().equals(XmlElementNames.DaysOfWeek)) {
          ...
          return true;
        } else if (reader.getLocalName().equals(XmlElementNames.DayOfWeekIndex)) {
    ...
    public DayOfTheWeekIndex getDayOfTheWeekIndex() throws ServiceValidationException {
      return this.getFieldValueOrThrowIfNull(DayOfTheWeekIndex.class, this.dayOfTheWeekIndex, "DayOfTheWeekIndex");
    }
      ...
      if (super.tryReadElementFromXml(reader)) {
        ...
        if (reader.getLocalName().equals(XmlElementNames.DaysOfWeek)) {
          ...
          this.firstDayOfWeek = reader.
                  readElementValue(Calendar.class, XmlNamespace.Types, XmlElementNames.FirstDayOfWeek);
          return true;
        } else {
