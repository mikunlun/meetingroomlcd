...
public final class FindItemResponse<TItem extends Item> extends ServiceResponse {
  ...
  protected void readElementsFromXml(EwsServiceXmlReader reader) throws Exception {
    ...
    int totalItemsInView = reader.readAttributeValue(Integer.class, XmlAttributeNames.TotalItemsInView);
    boolean moreItemsAvailable = !reader.readAttributeValue(Boolean.class, XmlAttributeNames.IncludesLastItemInRange);

    // Ignore IndexedPagingOffset attribute if moreItemsAvailable is false.
    Integer nextPageOffset = moreItemsAvailable ? reader.readNullableAttributeValue(Integer.class, XmlAttributeNames.IndexedPagingOffset) : null;
    ...
    if (!reader.isEmptyElement()) {
      do {
        ...
        if (reader.getNodeType().nodeType == XMLNodeType.START_ELEMENT) {
          Item item = EwsUtilities.createEwsObjectFromXmlElementName(Item.class, reader.getService(), reader.getLocalName());
