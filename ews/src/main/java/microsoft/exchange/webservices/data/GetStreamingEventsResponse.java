  ...
  protected boolean loadExtraErrorDetailsFromXml(EwsServiceXmlReader reader, String xmlElementName) throws Exception {
    ...
    if (reader.isStartElement(XmlNamespace.Messages, XmlElementNames.ErrorSubscriptionIds)) {
      do {
        ...
        if (reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT && reader.getLocalName() == XmlElementNames.SubscriptionId) {
          this.getErrorSubscriptionIds().add(reader.readElementValue(XmlNamespace.Messages, XmlElementNames.SubscriptionId));
        }
