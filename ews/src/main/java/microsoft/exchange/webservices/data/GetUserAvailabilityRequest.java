...
final class GetUserAvailabilityRequest extends SimpleServiceRequestBase {
  ...
  protected boolean isFreeBusyViewRequested() {
    return this.requestedData == AvailabilityData.FreeBusy || this.requestedData == AvailabilityData.
  ...
  protected boolean isSuggestionsViewRequested() {
    return this.requestedData == AvailabilityData.Suggestions || this.requestedData == AvailabilityData.
            FreeBusyAndSuggestions;
  }
  ...
  protected Object parseResponse(EwsServiceXmlReader reader) throws Exception {
    ...
    if (this.isFreeBusyViewRequested()) {
      ...
      do {
        ...
        if (reader.isStartElement(XmlNamespace.Messages, XmlElementNames.FreeBusyResponse)) {
          ...
          if (freeBusyResponse.getErrorCode().equals(ServiceError.NoError)) {
    ...
    if (this.isSuggestionsViewRequested()) {
      ...
      serviceResponse.getSuggestionsResponse().loadFromXml(reader, XmlElementNames.ResponseMessage);

      if (serviceResponse.getSuggestionsResponse().getErrorCode().equals(ServiceError.NoError)) {
