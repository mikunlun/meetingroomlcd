...
class SoapFaultDetails {
   ...
   */
  protected static SoapFaultDetails parse(EwsXmlReader reader, XmlNamespace soapNamespace) throws Exception {
   ...
   * @throws Exception          the exception
   */
  private void parseDetailNode(EwsXmlReader reader) throws ServiceXmlDeserializationException, XMLStreamException, Exception, Exception {
    do {
      reader.read();
      if (reader.getNodeType().equals(new XMLNodeType(XMLNodeType.START_ELEMENT))) {
        String localName = reader.getLocalName();
        if (localName.equals(XmlElementNames.EwsResponseCodeElementName)) {
          try {
            ...
            // as an ISE
            this.setResponseCode(ServiceError.
          ...
          try {
            ...
            // ServiceError couldn't be mapped to enum value, treat
            // as an ISE
            this.setErrorCode(ServiceError.
                    ErrorInternalServerError);
          }
