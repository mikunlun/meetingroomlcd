...
public abstract class SyncResponse<TServiceObject extends ServiceObject, TChange extends Change> extends ServiceResponse {
  ...
  protected void readElementsFromXml(EwsServiceXmlReader reader) throws ServiceLocalException, Exception {
    this.changes.setSyncState(reader.readElementValue(XmlNamespace.Messages, XmlElementNames.SyncState));
    this.changes.setMoreChangesAvailable(!reader.readElementValue(Boolean.class, XmlNamespace.Messages, this.getIncludesLastInRangeXmlElementName()));
    ...
    if (!reader.isEmptyElement()) {
      do {
        ...
        if (reader.isStartElement()) {
          ...
          } else if (reader.getLocalName().equals(XmlElementNames.Update)) {
          ...
          } else if (reader.getLocalName().equals(XmlElementNames.ReadFlagChange)) {
          ...
          if (change != null) {
            ...
            if (change.getChangeType().equals(ChangeType.Delete) || change.getChangeType().equals(ChangeType.ReadFlagChange)) {
              change.setId(change.createId());
              change.getId().loadFromXml(reader, change.getId().getXmlElementName());
              ...
              if (change.getChangeType().equals(ChangeType.ReadFlagChange)) {
                ...
                itemChange.setIsRead(reader.readElementValue(Boolean.class, XmlNamespace.Types, XmlElementNames.IsRead));
              }
            } else {

              change.setServiceObject(EwsUtilities.createEwsObjectFromXmlElementName(null, reader.getService(), reader.getLocalName()));

              change.getServiceObject().loadFromXml(reader, true, /* clearPropertyBag */
