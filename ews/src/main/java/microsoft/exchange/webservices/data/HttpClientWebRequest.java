...
class HttpClientWebRequest extends HttpWebRequest {
  ...
  public void prepareConnection() throws EWSHttpException {
    ...
    if (getProxy() != null) {
      ...
      if (HttpProxyCredentials.isProxySet()) {
        ...
        client.getState().setProxyCredentials(authScope, new NTCredentials(HttpProxyCredentials.getUserName(), HttpProxyCredentials.getPassword(), "", HttpProxyCredentials.getDomain()));
    ...
    if (getHeaders().size() > 0) {
      for (Map.Entry httpHeader : getHeaders().entrySet()) {
        httpMethod.setRequestHeader((String) httpHeader.getKey(), (String) httpHeader.getValue());
      }
   ...
   *
   * @throws EWSHttpException throws EWSHttpException
   */
  ...
  public InputStream getInputStream() throws EWSHttpException, IOException {
    ...
    try {
      bufferedInputStream = new BufferedInputStream(httpMethod.getResponseBodyAsStream());
    } catch (IOException e) {
   ...
   *
   * @param certs the Trust Manager
   * @throws EWSHttpException         the eWS http exception
   * @throws KeyManagementException   the KeyManagementException
